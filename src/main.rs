...
async fn weather_forecast_task(state: Arc<AppState>) {
    ...
    loop {
        ...
        let lats: Vec<String> = zones.iter().map(|(_, _, lat, _)| lat.to_string()).collect();
        let lons: Vec<String> = zones.iter().map(|(_, _, _, lon)| lon.to_string()).collect();
